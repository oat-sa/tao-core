...
class tao_helpers_translation_SourceCodeExtractor extends tao_helpers_translation_TranslationExtractor
{
     ...
     */
    public function setBannedFileType($bannedFileType): void
    {
    ...
    protected function getTranslationPhrases($line)
    {
        ...
        if (! empty($patternMatch2[1])) {
            return array_merge($strings, $patternMatch2[1]);
        }
     ...
     */
    private function recursiveSearch($directory): void
    {
        if (is_dir($directory)) {
     ...
     */
    private function getTranslationsInFile($filePath): void
    {

        // File extension ?
        $extOk = in_array(\Jig\Utils\FsUtils::getFileExtension($filePath), $this->getFileTypes(), true);
