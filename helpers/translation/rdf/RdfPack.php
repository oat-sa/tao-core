...
class RdfPack implements \IteratorAggregate
{
    ...
    protected function getTriplesFromFile($file, $modelId)
    {
        ...
        foreach ($translationFile->getTranslationUnits() as $tu) {
            ...
            $about = $annotations['po-translator-comments'] ?? null;
            if ($about && common_Utils::isUri($about) && in_array($tu->getContext(), [OntologyRdfs::RDFS_LABEL, OntologyRdfs::RDFS_COMMENT], true)) {
