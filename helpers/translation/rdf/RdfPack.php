...
class RdfPack implements \IteratorAggregate
{
     ...
     */
    public function __construct($langCode, \common_ext_Extension $extension, $modelId)
    {
    ...
    public function getIterator()
    {
        ...
        if ($this->langCode != 'en-US') {
            foreach ($this->extension->getManifest()->getInstallModelFiles() as $rdfpath) {
                $candidate = $this->extension->getDir(
                    ) . 'locales' . DIRECTORY_SEPARATOR . $this->langCode . DIRECTORY_SEPARATOR . basename(
                        $rdfpath
                    ) . '.po';
                if (file_exists($candidate)) {
                    $iterator->append($this->getTriplesFromFile($candidate));
                }
            }
        }
        return $iterator;
    ...
    protected function getTriplesFromFile($file)
    {
        ...
        foreach ($translationFile->getTranslationUnits() as $tu) {
            ...
            if ($about && common_Utils::isUri($about) && in_array(
                    $tu->getContext(),
                    array(
