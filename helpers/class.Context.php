...
class tao_helpers_Context
{
     ...
     */
    public static function load($mode): void
    {
        if (! is_string($mode)) {
        ...
        if (! in_array($mode, self::$current, true)) {
            self::$current[] = $mode;
        }
    }
    ...
    public static function check($mode)
    {
        ...
        $returnValue = in_array($mode, self::$current, true);
     ...
     */
    public static function unload($mode): void
    {
        if (! is_string($mode)) {
        ...
        if (in_array($mode, self::$current, true)) {
            $index = array_search($mode, self::$current, true);
            if ($index !== false) {
