...
class tao_helpers_Http
{
    ...
    static public function getStream()
    {
        if (!isset(self::$stream)){
            ...
            if (!isset($headers['Content-Length'])) {
                //unset($putData);
                throw new common_exception_Error("Incorrect Http headers, must have Content-Length", 411); // 411 Length Required
            }

            if (!isset($headers['Content-Md5'])) {
                //unset($putData);
                throw new common_exception_Error("Incorrect Http headers, must have Content-Md5", 449); // 449 Retry With
            }
     ...
     */
    static public function getJsonDataFromStream()
    {
