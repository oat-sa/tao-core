...
})}};return Pluginifier.register(ns,Adder,{expose:["add"]}),function($container){new DataAttrHandler("add",{container:$container,listenerEvent:"click",namespace:dataNs}).init(function($elt,$target){$elt.adder({target:$target,bindEvent:!1,content:$($elt.attr("data-content"))})}).trigger(function($elt){$elt.adder("add")})}}),define("context",["module"],function(module){return module.config()}),define("core/format",["lodash"],function(_){"use strict";var pattern=/(%[sdj])/g;return function(message){var replacements=Array.prototype.slice.call(arguments,1);return _.reduce(message.match(pattern),function(acc,match,index){var replacement="";if(replacements[index]){switch(match){
...
}}),define("layout/version-warning",["jquery","jquery.cookie"],function($){"use strict";function hideWarning(slide){var callback=function(){document.documentElement.className+=" no-version-warning",versionWarning.trigger("hiding.versionwarning")};slide?versionWarning.slideUp("slow",function(){versionWarning.slideUp("slow",callback)}):(versionWarning.hide(),callback())}var versionWarning=$(".version-warning");return{init:function(){return $.cookie("versionWarning")?void hideWarning(!1):void versionWarning.find(".close-trigger").on("click",function(){$.cookie("versionWarning",!0,{path:"/"}),hideWarning(!0)})}}}),define("controller/login",["jquery","ui","i18n","module","ui/feedback","layout/version-warning"],function($,ui,__,module,feedback,versionWarning){
"use strict";var conf,type,context=$(".entry-pages-container"),$fields=$();if(ui.startEventComponents($(".content-wrap")),versionWarning.init(),context.find(".form-error").length?(conf={message:{error:__("All fields are required")}},context.find(":input").each(function(){this.value||($fields=$fields.add($(this)))})):conf=module.config(),conf.message){$fields=context.find(":input");for(type in conf.message)conf.message[type]&&(feedback()[type](conf.message[type]),$fields.addClass(type))}}),function(window){"use strict";var appConfig=document.getElementById("amd-loader").getAttribute("data-config");require([appConfig],function(){require(["controller/login"])})}(window),
