    ...
    return {
      ...
      copyTo: function copyTo(
        ...
        if (_.isEmpty(uri)) {
          return Promise.reject(
            "The URI of the resource to copy must be defined"
          );
        }
        if (_.isEmpty(destinationClassUri)) {
          return Promise.reject(
            "The URI of the destination class must be defined"
          );
        }
      ...
      moveTo: function moveTo(ids, destinationClassUri, aclTransferMode) {
        ...
        if (_.isEmpty(ids) || _.some(ids, _.isEmpty)) {
          return Promise.reject(
            "The URI of the resource to move must be defined"
          );
        }
        if (_.isEmpty(destinationClassUri)) {
          return Promise.reject(
