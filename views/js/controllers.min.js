...
}),nav.init(),search.init(),section.on("activate",function(section){if(window.scrollTo(0,0),section&&section.id)switch(context.section=section.id,actions.init(section.panel),section.type){case"tree":section.panel.addClass("content-panel"),sectionHeight.init(section.panel),$(".taotree",section.panel).each(function(){var $treeElt=$(this),$actionBar=$(".tree-action-bar-box",section.panel),rootNode=$treeElt.data("rootnode"),treeUrl=context.root_url;treeUrl+=/\/$/.test(treeUrl)?$treeElt.data("url").replace(/^\//,""):$treeElt.data("url"),treeFactory($treeElt,treeUrl,{serverParameters:{extension:context.shownExtension,perspective:context.shownStructure,section:context.section,classUri:rootNode?rootNode:void 0},actions:{selectClass:$treeElt.data("action-selectclass"),selectInstance:$treeElt.data("action-selectinstance"),moveInstance:$treeElt.data("action-moveinstance"),"delete":$treeElt.data("action-delete")}}),$treeElt.on("ready.taotree",function(){$actionBar.addClass("active"),sectionHeight.setHeights(section.panel)
