...
},error:function(){feedback().error(__("Unable to release the lock"))}})})}}}),define("controller/main",["jquery","i18n","context","helpers","uiForm","layout/section","layout/actions","layout/tree","layout/version-warning","layout/section-height","layout/loading-bar","layout/nav","layout/search"],function($,__,context,helpers,uiForm,section,actions,treeFactory,versionWarning,sectionHeight,loadingBar,nav,search){"use strict";return{start:function(){var $doc=$(document);versionWarning.init(),$doc.ajaxSend(function(){loadingBar.start()}),$doc.ajaxComplete(function(){loadingBar.stop()}),nav.init(),search.init(),section.on("activate",function(section){if(window.scrollTo(0,0),
section&&section.id)switch(context.section=section.id,actions.init(section.panel),section.type){case"tree":section.panel.addClass("content-panel"),sectionHeight.init(section.panel),$(".taotree",section.panel).each(function(){var $treeElt=$(this),$actionBar=$(".tree-action-bar-box",section.panel),rootNode=$treeElt.data("rootnode"),treeUrl=context.root_url,treeActions={};$.each($treeElt.data("actions"),function(key,val){actions.getBy(val)&&(treeActions[key]=val)}),treeUrl+=/\/$/.test(treeUrl)?$treeElt.data("url").replace(/^\//,""):$treeElt.data("url"),treeFactory($treeElt,treeUrl,{serverParameters:{extension:context.shownExtension,perspective:context.shownStructure,
