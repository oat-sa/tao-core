...
!1)})}}}),define("controller/users/add",["jquery","module","helpers","users"],function($,module,helpers,users){"use strict";return{start:function(){var conf=module.config(),url=helpers._url("checkLogin","Users","tao");users.checkLogin(conf.loginId,url),conf.exit===!0&&setTimeout(function(){window.location=helpers._url("index","Main","tao",{structure:"users",ext:"tao",section:"list_users"})},1e3)}}}),define("controller/users/disable-edit",["layout/section"],function(section){"use strict";return{start:function(){section.get("edit_user").disable()}}}),define("controller/users/index",["module","jquery","i18n","util/url","layout/section","ui/feedback","ui/dialog/confirm","ui/datatable"],function(module,$,__,urlHelper,section,feedback,dialogConfirm){
"use strict";var runUserAction=function(uri,action,confirmMessage){var tokenName=module.config().xsrfTokenName,data={};data.uri=uri,data[tokenName]=$.cookie(tokenName),dialogConfirm(confirmMessage,function(){$.ajax({url:urlHelper.route(action,"Users","tao"),data:data,type:"POST"}).done(function(response){response.success?feedback().success(response.message):feedback().error(response.message),$("#user-list").datatable("refresh")})})},editUser=function(uri){section.get("edit_user").enable().loadContentBlock(urlHelper.route("edit","Users","tao"),{uri:uri}).show()},removeUser=function(uri,row){runUserAction(uri,"delete",__("Please confirm deletion of user %s",row.login));
},lockUser=function(uri,row){runUserAction(uri,"lock",__("Please confirm locking of account %s",row.login))},unlockUser=function(uri,row){runUserAction(uri,"unlock",__("Please confirm unlocking of account %s",row.login))};return{start:function(){var $userList=$("#user-list");section.on("show",function(section){"list_users"===section.id&&$userList.datatable("refresh")});var actions={edit:editUser,remove:removeUser,lock:lockUser,unlock:unlockUser};$userList.on("load.datatable",function(e,dataset){_.forEach(dataset.data,function(row){var lockBtn='[data-item-identifier="'+row.id+'"] button.lock',unlockBtn='[data-item-identifier="'+row.id+'"] button.unlock';row.lockable?$(row.locked?lockBtn:unlockBtn,$userList).hide():_.forEach([lockBtn,unlockBtn],function(btn){
$(btn,$userList).hide()})})}).datatable({url:urlHelper.route("data","Users","tao"),paginationStrategyBottom:"pages",filter:!0,actions:actions,model:[{id:"login",label:__("Login"),sortable:!0},{id:"firstname",label:__("First Name"),sortable:!0},{id:"lastname",label:__("Last Name"),sortable:!0},{id:"email",label:__("Email"),sortable:!0},{id:"roles",label:__("Roles"),sortable:!1},{id:"dataLg",label:__("Data Language"),sortable:!0},{id:"guiLg",label:__("Interface Language"),sortable:!0},{id:"status",label:__("Account status"),sortable:!0,transform:function(value){var icon="enabled"===value?"result-ok":"lock";return'<span class="icon-'+icon+'"></span> '+value}}]});
}}});
