...
},error:function(){feedback().error(__("Unable to release the lock"))}})})}}}),define("controller/main",["module","lodash","jquery","i18n","context","router","helpers","uri","uiForm","layout/generisRouter","layout/section","layout/actions","layout/tree","layout/version-warning","layout/section-height","layout/loading-bar","layout/nav","layout/search"],function(module,_,$,__,context,router,helpers,uri,uiForm,generisRouter,section,actions,treeFactory,versionWarning,sectionHeight,loadingBar,nav,search){"use strict";return{start:function(){var config=module.config(),$doc=$(document);versionWarning.init(),generisRouter.init(),$doc.ajaxSend(function(){loadingBar.start();
}),$doc.ajaxComplete(function(){loadingBar.stop()}),nav.init(),search.init(),section.on("activate",function(section){if(window.scrollTo(0,0),section&&section.id)switch(context.section=section.id,actions.init(section.panel),section.type){case"tree":section.panel.addClass("content-panel"),sectionHeight.init(section.panel),$(".taotree",section.panel).each(function(){var $treeElt=$(this),$actionBar=$(".tree-action-bar-box",section.panel),rootNode=$treeElt.data("rootnode"),treeUrl=context.root_url,treeActions={};$.each($treeElt.data("actions"),function(key,val){actions.getBy(val)&&(treeActions[key]=val)}),treeUrl+=/\/$/.test(treeUrl)?$treeElt.data("url").replace(/^\//,""):$treeElt.data("url"),
treeFactory($treeElt,treeUrl,{serverParameters:{extension:context.shownExtension,perspective:context.shownStructure,section:context.section,classUri:rootNode?rootNode:void 0},actions:treeActions,loadNode:uri.encode(section.defaultUri)}),$treeElt.on("ready.taotree",function(){$actionBar.addClass("active"),sectionHeight.setHeights(section.panel)})}),$(".navi-container",section.panel).show();break;case"content":this.loadContentBlock()}}).init(),helpers.init(),uiForm.init(),config&&_.isArray(config.extraRoutes)&&_.forEach(config.extraRoutes,function(route){router.dispatch(route)})}}}),define("controller/passwordRecovery",["jquery","i18n","module","ui/feedback","layout/version-warning"],function($,__,module,feedback,versionWarning){
