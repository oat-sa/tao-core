...
},error:function(){feedback().error(__("Unable to release the lock"))}})})}}}),define("controller/main",["module","lodash","jquery","i18n","context","router","helpers","uiForm","layout/section","layout/actions","layout/tree","layout/version-warning","layout/section-height","layout/loading-bar","layout/nav","layout/search"],function(module,_,$,__,context,router,helpers,uiForm,section,actions,treeFactory,versionWarning,sectionHeight,loadingBar,nav,search){"use strict";return{start:function(){var config=module.config(),$doc=$(document);versionWarning.init(),$doc.ajaxSend(function(){loadingBar.start()}),$doc.ajaxComplete(function(){loadingBar.stop()}),nav.init(),
search.init(),section.on("activate",function(section){if(window.scrollTo(0,0),section&&section.id)switch(context.section=section.id,actions.init(section.panel),section.type){case"tree":section.panel.addClass("content-panel"),sectionHeight.init(section.panel),$(".taotree",section.panel).each(function(){var $treeElt=$(this),$actionBar=$(".tree-action-bar-box",section.panel),rootNode=$treeElt.data("rootnode"),treeUrl=context.root_url,treeActions={};$.each($treeElt.data("actions"),function(key,val){actions.getBy(val)&&(treeActions[key]=val)}),treeUrl+=/\/$/.test(treeUrl)?$treeElt.data("url").replace(/^\//,""):$treeElt.data("url"),treeFactory($treeElt,treeUrl,{serverParameters:{
extension:context.shownExtension,perspective:context.shownStructure,section:context.section,classUri:rootNode?rootNode:void 0},actions:treeActions}),$treeElt.on("ready.taotree",function(){$actionBar.addClass("active"),sectionHeight.setHeights(section.panel)})}),$(".navi-container",section.panel).show();break;case"content":this.loadContentBlock()}}).init(),helpers.init(),uiForm.init(),config&&_.isArray(config.extraRoutes)&&_.forEach(config.extraRoutes,function(route){router.dispatch(route)})}}}),define("controller/passwordRecovery",["jquery","i18n","module","ui/feedback","layout/version-warning"],function($,__,module,feedback,versionWarning){"use strict";var feedbackType,conf=module.config();
if(versionWarning.init(),conf.message)for(feedbackType in conf.message)conf.message[feedbackType]&&feedback()[feedbackType](conf.message[feedbackType])}),define("controller/settings/extensionManager",["jquery","i18n","context","helpers","ui/feedback"],function($,__,context,helpers,feedback){function getDependencies(extension){var dependencies=[];return $("#"+extension+" .dependencies li:not(.installed)").each(function(){var ext=$(this).attr("rel"),deps=getDependencies(ext);deps.push(ext),dependencies=dependencies.concat(deps)}),dependencies}function getUnique(orig){for(var a=[],i=0;i<orig.length;i++)$.inArray(orig[i],a)<0&&a.push(orig[i]);return a}function progressConsole(msg){
