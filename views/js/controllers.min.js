define("controller/routes",[],function(){return{Main:{actions:{index:"controller/main",getSectionActions:"controller/main/actions",getSectionTrees:"controller/main/trees"}},ExtensionsManager:{actions:{index:"controller/settings/extensionManager"}},Lists:{actions:{index:"controller/list/index"}},RdfImExport:{actions:{index:"controller/RdfImExport/index"}},Users:{deps:"controller/users/disable-edit",actions:{index:"controller/users/index",add:"controller/users/add"}}}}),require(["jquery","lodash","i18n","helpers","ui/feedback","uiForm"],function($,_,__,helpers,feedback){var $form=$("#export"),$submitter=$form.find(".form-submitter"),$sent=$form.find(":input[name='"+$form.attr("name")+"_sent']");
$submitter.off("click").on("click",function(e){if(e.preventDefault(),parseInt($sent.val())){var $iframeContainer=$("#iframe-container"),params={},instances=[];if(_.each($form.serializeArray(),function(param){0===param.name.indexOf("rdftpl_ns_")?instances.push(param.name.substring(10)):params[param.name]=param.value}),!instances.length)return void feedback().error(__("Please select alteast one namespace"));params.namespaces=instances;var url=helpers._url("export","RdfImExport","tao",params),$iframe=$("<iframe>",{src:url}).hide();$iframeContainer.empty().append($iframe)}})}),define("tao/controller/RdfImExport/index",function(){}),define("tao/controller/list/index",["jquery","i18n","helpers","ui/feedback"],function($,__,helpers,feedback){function _addSquareBtn(title,icon,$listToolBar){var $btn=$("<button>",{"class":"btn-info small lft icon-"+icon,title:__(title)});
return $listToolBar.append($btn),$btn}return{start:function(){var saveUrl=helpers._url("saveLists","Lists","tao"),delListUrl=helpers._url("removeList","Lists","tao"),delEltUrl=helpers._url("removeListElement","Lists","tao");$(".list-edit-btn").click(function(){var $listSaveBtn,$listNewBtn,$btn=$(this),uri=$btn.data("uri"),$listContainer=$("#list-data_"+uri),$listForm=$listContainer.find("form"),$listTitleBar=$listContainer.find(".container-title h6"),$listToolBar=$listContainer.find(".data-container-footer");if(!$listForm.length){$listForm=$("<form>"),$listContainer.wrapInner($listForm),$listContainer.find("form").append('<input type="hidden" name="uri" value="'+uri+'" />');
var $labelEdit=$("<input type='text' name='label' value='"+$listTitleBar.text()+"'/>");$listTitleBar.html($labelEdit),$listContainer.find(".list-element").length&&$listContainer.find(".list-element").replaceWith(function(){return"<input type='text' name='"+$(this).attr("id")+"' value='"+$(this).text()+"' />"});var elementList=$listContainer.find("ol");elementList.addClass("sortable-list"),elementList.find("li").prepend('<span class="icon-grip" ></span>'),elementList.find("li").append('<span class="icon-checkbox-crossed list-element-delete-btn" style="cursor:pointer;" ></span>'),elementList.sortable({axis:"y",opacity:.6,placeholder:"ui-state-error",tolerance:"pointer",update:function(){var map={};
