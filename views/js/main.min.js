...
}),define("ui/tooltipster",["jquery","lodash","core/dataattrhandler","tooltipster"],function($,_,DataAttrHandler){var themes=["dark","default","info","warning","error","success"];return function($container){$("[data-tooltip]",$container).each(function(){var $elt=$(this),$target=DataAttrHandler.getTarget("tooltip",$elt),theme=_.contains(themes,$elt.data("tooltip-theme"))?$elt.data("tooltip-theme"):"default";$elt.tooltipster({theme:"tao-"+theme+"-tooltip",content:$target,contentAsHTML:$target.children().length>0,delay:350,trigger:"hover"})})}}),define("ui/form",["jquery"],function($){var pseudoLabel=function(selector){$(document).on("click",selector+" .pseudo-label-box",function(e){e.preventDefault();
...
})}),define("ui/feedback",["jquery","lodash","tpl!ui/feedback/feedback"],function($,_,tpl){function delegate(receiver,provider){return _(provider).functions().forEach(function(methodName){receiver[methodName]=function(){return provider[methodName].apply(receiver,arguments)}}),receiver}var $feedBackBox,currents=[],categories={"volatile":["info","success"],persistent:["warning","error"]},levels=_(categories).values().flatten().value(),states={created:"created",displayed:"displayed",closed:"closed",merged:"merged",collapsed:"collapsed"},defaultOptions={timeout:{info:2e3,success:2e3,warning:4e3,error:8e3}},feedbackApi={level:null,category:null,message:function(level,msg,options){return level&&_.contains(levels,level)||(level="info"),this.setState(states.created),this.level=level,this.category=_.findKey(categories,[this.level]),this.options=_.defaults(options||{},defaultOptions),this.content=tpl({level:level,msg:msg}),this._trigger("create"),this
