...
return _.isFunction(arg1)?callback=arg1:_.isObject(arg1)&&(_.merge(options,arg1),_.isFunction(arg2)&&(callback=arg2)),_.each(this.rules,function(rule){var validatorOptions={};callstack.push(function(cb){_applyRules(value,rule,function(success){if(success)cb(null,new Report("success",{validator:rule.name}));else{var report=new Report("failure",{validator:rule.name,message:rule.message});options.lazy?cb(new Error("lazy mode"),report):cb(null,report)}},validatorOptions)})}),async.series(callstack,function(err,results){_.isFunction(callback)&&callback(results)}),this},Validator.prototype.addRule=function(rule){var _rules=validators.validators;return _.isString(rule)&&_rules[rule]?this.rules.push(_rules[rule]):(rule=_buildRule(rule))&&this.rules.push(rule),this
},Validator.prototype.addRules=function(rules){var _this=this;return _.each(rules,function(rule){_this.addRule(rule)}),this},Validator}),define("ui/validator",["jquery","lodash","core/validator/Report","core/validator/Validator"],function($,_,Report,Validator){function isCreated($elt){return"object"==typeof $elt.data("validator-config")}$.fn.validator=function(options){var ret,opts={},method="",args=[];return"object"==typeof options?opts=$.extend({},$.fn.validator.defaults,options):void 0===options?opts=$.extend({},$.fn.validator.defaults):"string"==typeof options&&"function"==typeof methods[options]&&(method=options,args=Array.prototype.slice.call(arguments,1)),this.each(function(){var $this=$(this);
