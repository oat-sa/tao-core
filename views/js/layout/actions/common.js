  ...
  var commonActions = function commonActions() {
    ...
    binder.register("load", function load(actionContext) {
      section
        ...
        .loadContentBlock(
          this.url,
          _.pick(actionContext, ["uri", "classUri", "id"])
        );
    });
            ...
            request({
              ...
              if (response.success && response.deleted) {
                ...
                reject(
                  new Error(
                    response.message ||
                      __("Unable to delete the selected resources")
                  )
                );
              }
            });
    ...
    binder.register("launchEditor", function launchEditor(actionContext) {
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          if (
            $response.is(wideDifferenciator) ||
            $response.find(wideDifferenciator).length
          ) {
            section
              .create({
                id: "authoring",
      ...
      return new Promise(function (resolve, reject) {
        ...
        destinationSelectorFactory($container, {
          ...
          preventSelection: function preventSelection(nodeUri, node, $node) {
            ...
            if (
              ($node.length && $node.data("access") === "partial") ||
              $node.data("access") === "denied"
            ) {
              if (!permissionsManager.hasPermission(nodeUri, "WRITE")) {
                feedback().warning(
                  __(
                    "You are not allowed to write in the class %s",
                    node.label
                  ),
                  { encodeHtml: false }
                );
        ...
        const destinationSelector = destinationSelectorFactory($container, {
          ...
          .on("query", (params) => {
            ...
            resourceProvider
              .getResources(params, true)
              .then((resources) =>
                destinationSelector.update(resources, params)
              )
        ...
        destinationSelectorFactory($container, {
          ...
          preventSelection: function preventSelection(nodeUri, node, $node) {
            ...
            if (_.intersection(selectedUri, uriList).length) {
              feedback().warning(
                __(
                  "You cannot move the selected resources in the class %s",
                  node.label
                ),
                { encodeHtml: false }
              );
              ...
              resourceProvider
                ...
                .then(function (results) {
                  ...
                  if (!success.length) {
                    ...
                    feedback().warning(
                      __(
                        "Some resources have not been moved: %s",
                        failed.join(", ")
                      )
                    );
                  } else {
