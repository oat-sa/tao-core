...
;return buffer+="\n            ",((stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.options)||!1===stack1?stack1:stack1.emptyText,{hash:{},inverse:self.noop,fn:self.program(89,program89,data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+="\n        "},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+='\n    </div>\n    <div class="datatable-pagination-bottom"></div>\n</div>'})}),define("tpl!ui/datatable/tpl/button",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",
...
var stack1,helper;return stack1=(helper=helpers.label)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.label)===functionType?helper.call(depth0,{hash:{},data:data}):helper,escapeExpression(stack1)},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+="</button>"})}),define("tpl!ui/pagination/providers/tpl/pages",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],data=data||{};var helper,options,buffer="",helperMissing=(helpers=this.merge(helpers,Handlebars.helpers)).helperMissing,escapeExpression=this.escapeExpression
...
Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var helper,buffer="";return buffer+='<li class="page"><span>',buffer+=(0,this.escapeExpression)((helper=helpers.page)?helper.call(depth0,{hash:{},data:data}):"function"==typeof(helper=depth0&&depth0.page)?helper.call(depth0,{hash:{},data:data}):helper)+"</span></li>"})}),define("tpl!ui/pagination/providers/tpl/simple",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],data=data||{};var helper,options,buffer="",helperMissing=(helpers=this.merge(
...
"lib/jsTree/plugins/jquery.tree.checkbox"],function($,_,__,context,GenerisTreeClass,helpers,feedback){return GenerisTreeClass.extend({init:function(selector,dataUrl,options){this.loadedData=null,this.checkedNodes=void 0!==options.checkedNodes?options.checkedNodes.slice(0):[],this.hiddenNodes=void 0!==options.hiddenNodes?options.hiddenNodes.slice(0):[],options.callback&&options.callback.checkPaginate&&(this.checkPaginate=options.callback.checkPaginate);var instance=this,treeOptions={types:{default:{draggable:!(this.DISPLAY_SELECTED=1)}},ui:{theme_name:"checkbox",theme_path:context.taobase_www+"js/lib/jsTree/themes/css/style.css"},callback:{
beforecheck:function(NODE,TREE_OBJ){var nodeId=$(NODE).prop("id");return(!instance.isRefreshing||-1!==$.inArray(nodeId,instance.checkedNodes))&&(!NODE.hasClass("node-class")||instance.getMeta(nodeId,"displayed")===instance.getMeta(nodeId,"count")||(instance.paginateInstances(NODE,TREE_OBJ,{limit:0,checkedNodes:"*"}),!1))},beforeuncheck:function(NODE,TREE_OBJ){var nodeId=$(NODE).prop("id"),indice=$.inArray(nodeId,instance.checkedNodes);return!$(NODE).hasClass("node-class")&&-1<indice&&instance.checkedNodes.splice(indice,1),!0},beforedata:function(NODE,TREE_OBJ){var returnValue=instance.defaultServerParameters;for(var key in NODE&&(returnValue.classUri=$(NODE
).prop("id")),instance.serverParameters)null!==instance.serverParameters[key]&&(returnValue[key]=instance.serverParameters[key]);return returnValue},onopen:function(NODE,TREE_OBJ){instance.checkedNodes&&instance.check(instance.checkedNodes)},onload:function(TREE_OBJ){instance.check(instance.checkedNodes),instance.options.loadCallback&&instance.options.loadCallback(TREE_OBJ),instance.isRefreshing=!1},onchange:function(NODE,TREE_OBJ){instance.options.onChangeCallback&&!instance.isRefreshing&&instance.options.onChangeCallback(NODE,TREE_OBJ)},onselect:function(NODE,TREE_OBJ){if($(NODE).hasClass("paginate-more"))instance.paginateInstances($(NODE).parent().parent(
),TREE_OBJ);else{if(!$(NODE).hasClass("paginate-all"))return!0;var parentNodeId=$(NODE).parent().parent().prop("id"),limit=instance.getMeta(parentNodeId,"count")-instance.getMeta(parentNodeId,"displayed");instance.paginateInstances($(NODE).parent().parent(),TREE_OBJ,{limit:limit})}},ondata:function(DATA,TREE_OBJ){return DATA.children&&(DATA.state="open"),instance.loadedData=DATA,instance.extractMeta(DATA),instance.removeHiddenNodes(DATA.children||DATA),DATA}},plugins:{checkbox:{three_state:!0}}};for(var i in this.serverParameters)treeOptions.data.opts[i]=this.serverParameters[i];this._super(selector,dataUrl,options,treeOptions),$(
"#saver-action-"+this.options.actionId).click({instance:this},function(e){e.data.instance.saveData()})},removeHiddenNodes:function(nodes){var self=this,hiddenNodes=this.hiddenNodes;_.isArray(nodes)&&hiddenNodes&&_.isArray(hiddenNodes)&&_.remove(nodes,function(node){if("instance"===node.type)return 0<=_.indexOf(hiddenNodes,node.attributes["data-uri"]);"class"===node.type&&node.children&&self.removeHiddenNodes(node.children)})},trace:function(){},paginateInstances:function(NODE,TREE_OBJ,pOptions,callback){var instance,nodeId=NODE[0].id,instancesLeft=this.getMeta(nodeId,"count")-this.getMeta(nodeId,"displayed"),options={classUri:nodeId,subclasses:0,
offset:this.getMeta(nodeId,"position"),limit:instancesLeft<this.paginate?instancesLeft:this.paginate};options=$.extend(options,pOptions),$.post(this.dataUrl,options,(instance=this,function(DATA){instance.hidePaginate(NODE,TREE_OBJ);for(var i=0;i<DATA.length;i++)DATA[i].attributes.class=instance.options.instanceClass+" node-instance node-draggable",$("#"+DATA[i].attributes.id,$(TREE_OBJ.container)).length||TREE_OBJ.create(DATA[i],TREE_OBJ.get_node(NODE[0])),"*"==options.checkedNodes&&instance.checkedNodes.push(DATA[i].attributes.id);instance.setMeta(nodeId,"displayed",instance.getMeta(nodeId,"displayed")+DATA.length),instance.setMeta(nodeId,"position",
instance.getMeta(nodeId,"position")+DATA.length),instance.refreshPaginate(NODE,TREE_OBJ),options.checkedNodes&&("*"==options.checkedNodes?$(NODE).find("ul:first").children().each(function(){$(this).hasClass("node-instance")&&$(this).find("a:not(.checked, .undetermined)").each(function(){instance.checkedNodes.push($(this).parent().prop("id"))})}):instance.checkedNodes=options.checkedNodes),instance.check(instance.checkedNodes),callback&&callback(NODE,TREE_OBJ),instance.checkPaginate&&instance.checkPaginate(NODE,TREE_OBJ)}),"json")},check:function(elements){var self=this;$.each(elements,function(i,elt){if(null!=elt){var NODE=$(self.selector).find(
"li[id='"+elt+"']");0<NODE.length&&$(NODE).hasClass("node-instance")&&$.tree.plugins.checkbox.check(NODE)}})},getChecked:function(){var unchecked=[];$.each($.tree.plugins.checkbox.get_unchecked(this.getTree()),function(i,NODE){$(NODE).hasClass("node-instance")&&unchecked.push($(NODE).prop("id"))});var returnValue=$.grep(this.checkedNodes,function(value){return-1==unchecked.indexOf(value)});return $.each($.tree.plugins.checkbox.get_checked(this.getTree()),function(i,NODE){if($(NODE).hasClass("node-instance")){var value=$(NODE).prop("id");-1==returnValue.indexOf(value)&&returnValue.push(value)}}),returnValue},saveData:function(){var instance=this,toSend={};if(
"object"==typeof this.options.saveData)for(var key in this.options.saveData)toSend[key]=this.options.saveData[key];helpers.loading();var nodes=this.getChecked();toSend.instances=JSON.stringify(nodes);var uriField,classUriField=null;if(this.options.relatedFormId){var uriEltSelector="#"+this.options.relatedFormId+" :input[name=uri]";$(uriEltSelector).length&&(uriField=$(uriEltSelector));var classUriEltSelector="#"+this.options.relatedFormId+" :input[name=classUri]";$(classUriEltSelector).length&&(classUriField=$(classUriEltSelector))}uriField||(uriField=$("input[name=uri]")),classUriField||(classUriField=$("input[name=classUri]")),uriField&&(
