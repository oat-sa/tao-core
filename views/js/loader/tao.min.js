...
Promise,request,permissionsManager){"use strict";var defaultConfig={getClasses:{url:urlUtil.route("getAll","RestClass","tao")},getResources:{url:urlUtil.route("getAll","RestResource","tao")},getClassProperties:{url:urlUtil.route("create","RestResource","tao")},copyTo:{},moveTo:{}},computeNodeAccessMode=function computeNodeAccessMode(nodes){return _.map(nodes,function(node){return node.accessMode=permissionsManager.getResourceAccessMode(node.uri),_.isArray(node.children)&&(node.children=computeNodeAccessMode(node.children)),node})};function applyClassSignatures(resources,signature){return _.isArray(resources)?_.forEach(resources,function(resource){
applyClassSignatures(resource,signature)}):resources&&(signature&&(resources.classSignature=signature),resources.children&&applyClassSignatures(resources.children,resources.signature||signature)),resources}return function(config){return config=_.defaults(config||{},defaultConfig),{getClasses:function(classUri){return request(config.getClasses.url,{classUri:classUri})},getResources:function(params,computePermissions){return request(config.getResources.url,params).then(function(results){var resources,currentRights;return resources=results&&results.resources?results.resources:results,computePermissions&&results.permissions&&(
currentRights=permissionsManager.getRights(),results.permissions.supportedRights&&results.permissions.supportedRights.length&&0===currentRights.length&&permissionsManager.setSupportedRights(results.permissions.supportedRights),results.permissions.data&&permissionsManager.addPermissions(results.permissions.data),resources.nodes?resources.nodes=computeNodeAccessMode(resources.nodes):resources=computeNodeAccessMode(resources)),resources}).then(applyClassSignatures)},getClassProperties:function(classUri){return request(config.getClassProperties.url,{classUri:classUri})},copyTo:function(uri,destinationClassUri,signature){return _.isEmpty(config.copyTo.url
)?Promise.reject("Please define the action URL"):_.isEmpty(uri)?Promise.reject("The URI of the resource to copy must be defined"):_.isEmpty(destinationClassUri)?Promise.reject("The URI of the destination class must be defined"):request(config.copyTo.url,{uri:uri,destinationClassUri:destinationClassUri,signature:signature},"POST")},moveTo:function(ids,destinationClassUri){var params={destinationClassUri:destinationClassUri};return ids?_.isArray(ids)||(ids=[ids]):ids=[],1===ids.length?(params.uri=ids[0],params.signature=config.moveTo.signature):params.ids=ids,_.isEmpty(config.moveTo.url)?Promise.reject("Please define the action URL"):_.isEmpty(ids)||_.some(ids
,_.isEmpty)?Promise.reject("The URI of the resource to move must be defined"):_.isEmpty(destinationClassUri)?Promise.reject("The URI of the destination class must be defined"):request(config.moveTo.url,params,"POST")}}}}),define("tpl!ui/class/tpl/selector",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",functionType="function",escapeExpression=this.escapeExpression;return buffer+='<div class="class-selector">\n    ',((stack1=helpers.if.call(depth0,depth0&&depth0.classUri,{hash:{},
