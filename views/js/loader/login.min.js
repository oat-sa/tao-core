...
options={hash:{},data:data},helper?helper.call(depth0,"Remove Message",options):helperMissing.call(depth0,"__","Remove Message",options)))+'" class="icon-close" data-close=":parent .feedback"></span>\n</div>\n'})}),define("ui/feedback",["jquery","lodash","util/wrapLongWords","util/encode","tpl!ui/feedback/feedback"],function($,_,wrapLongWords,encode,tpl){"use strict";function delegate(receiver,provider){return _(provider).functions().forEach(function(methodName){receiver[methodName]=function(){return provider[methodName].apply(receiver,arguments)}}),receiver}var $feedBackBox,currents=[],categories={volatile:["info","success"],persistent:["warning","danger","error"]
},levels=_(categories).values().flatten().value(),states={created:"created",displayed:"displayed",closed:"closed",merged:"merged",collapsed:"collapsed"},defaultOptions={timeout:{info:2e3,success:2e3,warning:4e3,danger:4e3,error:8e3},wrapLongWordsAfter:40,encodeHtml:!0,popup:!0},feedbackApi={level:null,category:null,message:function(level,msg,options){return level&&_.contains(levels,level)||(level="info"),this.setState(states.created),this.level=level,this.category=_.findKey(categories,[this.level]),this.options=_.defaults(options||{},defaultOptions),msg=this.options.encodeHtml?encode.html(msg):msg,msg=this.options.wrapLongWordsAfter?wrapLongWords(msg,this.options.wrapLongWordsAfter):msg,
this.content=tpl({level:level,msg:msg,popup:!!this.options.popup}),this._trigger("create"),this},info:function(msg,options){return this.message("info",msg,options).open()},success:function(msg,options){return this.message("success",msg,options).open()},warning:function(msg,options){return this.message("warning",msg,options).open()},danger:function(msg,options){return this.message("danger",msg,options).open()},error:function(msg,options){return this.message("error",msg,options).open()},open:function(){return this._trigger(),_(currents).reject({id:this.id}).invoke("close"),this.display("open"),this},close:function(){this.isInState(states.displayed)&&(this.setState(states.closed),
$("#"+this.id).remove(),this._trigger("close"),_.remove(currents,{_state:states.closed}))},display:function(){var self=this;if(this.content){this.setState(states.displayed),$(this.content).attr("id",this.id).appendTo(this._container),this._trigger("display"),this._getTimeout()>=0&&setTimeout(function(){self.close()},this._getTimeout());var $btn=this._container.find(".icon-close");$btn.length&&$btn.off("click").on("click",self.close)}return this},merge:function(){var previous=_.find(currents,{category:"persistent"});return previous?(this.setState(states.merged),void this._trigger("merge")):this.display()},collapse:function(){this._trigger("collapse")},_trigger:function(eventName){
var name=eventName;this._container.trigger(name+".feedback",[this]),_.isFunction(this.options[name])&&this.options[name].call(this)},_getTimeout:function(level){return _.isUndefined(level)&&(level=this.level),_.isObject(this.options.timeout)?this.options.timeout[level]:this.options.timeout}},feedbackState={_state:null,isInState:function(verify){return _.isString(verify)||(verify=[verify]),_.contains(verify,this._state)},setState:function(state){if(!_.contains(states,state))throw new Error("Unkown state "+state);this._state=state}},feedbackFactory=function($container){var _container;if($feedBackBox||($feedBackBox=$("#feedback-box")),_container=$container||$feedBackBox,
!_container||!_container.length)throw new Error("The feedback needs to belong to an existing container");var fb=_.extend({id:"feedback-"+(currents.length+1),_container:_container},feedbackState);return currents.push(fb),delegate(fb,feedbackApi)};return feedbackFactory}),define("jquery.cookie",["jquery"],function(jQuery){jQuery.cookie=function(name,value,options){if("undefined"==typeof value){var cookieValue=null;if(document.cookie&&""!=document.cookie)for(var cookies=document.cookie.split(";"),i=0;i<cookies.length;i++){var cookie=jQuery.trim(cookies[i]);if(cookie.substring(0,name.length+1)==name+"="){cookieValue=decodeURIComponent(cookie.substring(name.length+1));
