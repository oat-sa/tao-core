...
},getLevelNum=function(level){return _.isString(level)&&_.has(levels,level)?levels[level]:_.isNumber(level)&&_.contains(levels,level)?level:levels[defaultLevel]},checkMinLevel=function(minLevel,level){return getLevelNum(level)>=getLevelNum(minLevel)},loggerFactory=function loggerFactory(name,minLevel,fields){var baseRecord,logger;if(!_.isString(name)||_.isEmpty(name))throw new TypeError("A logger needs a name");return baseRecord=_.defaults(fields||{},{name:name,pid:1,hostname:navigator.userAgent}),logger={log:function(level,recordFields,message){var record,err,rest=[],time=(new Date).toISOString();if(loggerFactory.providers!==!1&&checkMinLevel(minLevel||defaultLevel,level))return _.isString(recordFields)||recordFields instanceof Error?(message=recordFields,
recordFields={},rest=[].slice.call(arguments,2)):rest=[].slice.call(arguments,3),record={level:getLevel(level),v:bunyanVersion,time:time},checkMinLevel(levels.error,level)||message instanceof Error?(err=message instanceof Error?message:new Error(message),record.msg=err.message,record.err=err):record.msg=format.apply(null,[message].concat(rest)),_.merge(record,baseRecord,recordFields),logQueue.push(record),loggerFactory.flush(),this},level:function(value){return"undefined"!=typeof value?(minLevel=getLevelNum(value),this):getLevel(minLevel)},child:function(childFields){return loggerFactory(name,minLevel,_.defaults(childFields,baseRecord))}},_.reduce(levels,function(target,level,levelName){
return target[levelName]=_.partial(logger.log,level),target},logger)};return loggerFactory.levels=levels,loggerFactory.providers=!1,loggerFactory.load=function(modules){var self=this;return this.providers=[],new Promise(function(resolve,reject){require(modules,function(){var loadedProviders=[].slice.call(arguments);_.forEach(loadedProviders,function(provider){try{self.register(provider)}catch(err){reject(err)}}),self.flush(),resolve()},reject)})},loggerFactory.register=function(provider){if(!_.isPlainObject(provider)||!_.isFunction(provider.log))throw new TypeError("A log provider is an object with a log method");this.providers=this.providers||[],this.providers.push(provider);
},loggerFactory.flush=function(){_.isArray(this.providers)&&this.providers.length>0&&(_.forEach(logQueue,function(message){_.forEach(loggerFactory.providers,function(provider){provider.log.call(provider,message)})}),logQueue=[])},loggerFactory.setDefaultLevel=function(level){return defaultLevel=getLevel(level)},loggerFactory}),define("core/logger",["lodash","module","core/logger/api"],function(_,module,loggerFactory){"use strict";var defaultConfig={level:loggerFactory.levels.warn,loggers:["core/logger/console"]},config=_.defaults(module.config()||{},defaultConfig);return _.isArray(config.loggers)&&config.loggers.length&&(loggerFactory.setDefaultLevel(config.level),
loggerFactory.load(config.loggers)),window.setTaoLogLevel=function(level){return loggerFactory.setDefaultLevel(level)},loggerFactory}),define("core/eventifier",["lodash","async","core/promise","lib/uuid","core/logger"],function(_,async,Promise,uuid,loggerFactory){"use strict";function createAsyncCallstack(handlers,context,eventName,namespace,args,success){var callstack=_.map(handlers,function(handler){return function(cb){function done(){cb(null,{success:!0})}function prevent(){cb(null,{success:!1})}function preventNow(){cb(new Error("prevent now"),{success:!1,immediate:!0})}var result,asyncMode=!1,_args=_.clone(args),event={name:eventName,namespace:namespace,
done:function(){return asyncMode=!0,done},prevent:function(){asyncMode=!0,prevent()},preventNow:function(){asyncMode=!0,preventNow()}};_args.unshift(event),result=handler.apply(context,_args),asyncMode||(result===!1?prevent():Promise.resolve(result).then(done).catch(prevent))}});async.series(callstack,function(err,results){var successes=_.pluck(results,"success");_.indexOf(successes,!1)===-1&&success()})}function getEventNames(eventNames){return!_.isString(eventNames)||_.isEmpty(eventNames)?[]:_(eventNames.split(/\s/g)).compact().uniq().value()}function getName(eventName){return eventName.indexOf(".")>-1?eventName.substr(0,eventName.indexOf(".")):eventName}function getNamespace(eventName){
return eventName.indexOf(".")>-1?eventName.substr(eventName.indexOf(".")+1):defaultNs}function getHandlerObject(){return{before:[],between:[],after:[]}}function eventifier(target){var targetName,logger,eventHandlers={},getHandlers=function(eventName,type){var name=getName(eventName),ns=getNamespace(eventName);return type=type||"between",eventHandlers[ns]=eventHandlers[ns]||{},eventHandlers[ns][name]=eventHandlers[ns][name]||getHandlerObject(),eventHandlers[ns][name][type]},eventApi={on:function(eventNames,handler){return _.isFunction(handler)&&_.forEach(getEventNames(eventNames),function(eventName){getHandlers(eventName).push(handler)}),this},off:function(eventNames){
return _.forEach(getEventNames(eventNames),function(eventName){var offNamespaces,name=getName(eventName),ns=getNamespace(eventName);ns&&!name?ns===globalNs?(offNamespaces={},offNamespaces[defaultNs]=eventHandlers[defaultNs],eventHandlers=offNamespaces):eventHandlers[ns]={}:_.forEach(eventHandlers,function(nsHandlers,namespace){!nsHandlers[name]||ns!==defaultNs&&ns!==namespace||(nsHandlers[name]=getHandlerObject())})}),this},removeAllListeners:function(){return eventHandlers={},this},trigger:function(eventNames){function triggerEvent(handlers){_.forEach(handlers.between,function(handler){handler.apply(self,args)}),_.forEach(handlers.after,function(handler){handler.apply(self,args);
})}var self=this,args=[].slice.call(arguments,1);return _.forEach(getEventNames(eventNames),function(eventName){var ns=getNamespace(eventName),name=getName(eventName),mergedHandlers=_(eventHandlers).filter(function(nsHandlers,namespace){return nsHandlers[name]&&(ns===defaultNs||ns===namespace||namespace===globalNs)}).reduce(function(acc,nsHandlers){return acc.before=acc.before.concat(nsHandlers[name].before),acc.between=acc.between.concat(nsHandlers[name].between),acc.after=acc.after.concat(nsHandlers[name].after),acc},getHandlerObject());logger.debug({event:eventName,args:args},"trigger %s",eventName),mergedHandlers&&(mergedHandlers.before.length?createAsyncCallstack(mergedHandlers.before,self,name,ns,args,_.partial(triggerEvent,mergedHandlers)):triggerEvent(mergedHandlers));
}),this},before:function(eventNames,handler){return _.isFunction(handler)&&_.forEach(getEventNames(eventNames),function(eventName){getHandlers(eventName,"before").push(handler)}),this},after:function(eventNames,handler){return _.isFunction(handler)&&_.forEach(getEventNames(eventNames),function(eventName){getHandlers(eventName,"after").push(handler)}),this}};return target=target||{},targetName=target.name||target.id||target.serial||uuid(6),logger=eventifierLogger.child({target:targetName}),_(eventApi).functions().forEach(function(method){target[method]=function(){var args=[].slice.call(arguments);return eventApi[method].apply(target,args)}}),target}var defaultNs="@",globalNs="*",eventifierLogger=loggerFactory("eventifier");
