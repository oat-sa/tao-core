...
class CsvImportTest extends TaoPhpUnitTestRunner
{
    ...
    public function testImport(): void
    {
        ...
        $this->assertSame(16, count($report->getSuccesses()));
        $this->assertSame(common_report_Report::TYPE_SUCCESS, $report->getType());
        $this->assertFalse($file->exists());
    }

    public function testCsvMapping(): void
    {
        $importer = new CsvBasicImporter();
        ...
        $this->assertSame($expectedHeaderMap, $map['headerList']);
        ...
        $this->assertSame('Label', $map['classProperties'][OntologyRdfs::RDFS_LABEL]);
        $this->assertSame('lAst naMe', $map['classProperties']['uriproperty1']);
        $this->assertSame('Login', $map['classProperties']['uriproperty2']);
        $this->assertSame('labelproperty3', $map['classProperties']['http://tao.unit/test.rdf#fIrstnAmE']);
        $this->assertSame('labelproperty4', $map['classProperties']['http://tao.unit/test.rdf#email']);
        $this->assertCount(5, $map['mapping']);
        $this->assertSame(0, $map['mapping'][OntologyRdfs::RDFS_LABEL]);
        $this->assertSame(2, $map['mapping']['uriproperty1']);
        $this->assertSame(3, $map['mapping']['uriproperty2']);
        $this->assertSame(1, $map['mapping']['http://tao.unit/test.rdf#fIrstnAmE']);
        $this->assertSame(4, $map['mapping']['http://tao.unit/test.rdf#email']);
    }

    public function testGetDataSample(): void
    {
        $importer = new CsvBasicImporter();
        ...
        foreach ($sample as $i => $row) {
            ...
            $this->assertSame($expectedLogin[$i], $row['Login']);
        }
    ...
    public function testGetClassProperties(): void
    {
        ...
        $this->assertSame($propertiesExpected, $properties);
    }
    ...
    public function testImportRules(): void
    {
        ...
        $this->assertInstanceOf('common_report_Report', $report);
        $this->assertSame(common_report_Report::TYPE_WARNING, $report->getType());
