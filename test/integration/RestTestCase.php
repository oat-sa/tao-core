...
abstract class RestTestCase extends RestTestRunner
{
     ...
     */
    public function testHttp($service): void
    {
        ...
        $this->assertSame('401', $http_status, 'bad response on url ' . $url . ' return ' . $http_status);
        curl_close($process);
        ...
        $data = curl_exec($process);
        $http_status = curl_getinfo($process, CURLINFO_HTTP_CODE);
        $this->assertSame($http_status, '401');
        curl_close($process);
        ...
        $data = curl_exec($process);
        $http_status = curl_getinfo($process, CURLINFO_HTTP_CODE);
        $this->assertSame($http_status, '406');
        curl_close($process);
        ...
        $data = curl_exec($process);
        $http_status = curl_getinfo($process, CURLINFO_HTTP_CODE);
        $this->assertSame($http_status, '200');
        ...
        $this->assertSame($contentType, 'application/xml');
        curl_close($process);
     ...
     */
    public function testGetAll($service, $topclass = null): void
    {
        if ($topclass === null) {
        ...
        foreach ($data['data'] as $results) {
            ...
            foreach ($results['properties'] as $propArray) {
                ...
                if (\common_Utils::isUri(current($values))) {
                    $this->assertSame('resource', $current['valueType']);
                } else {
                    $this->assertSame('literal', $current['valueType']);
                }
                $this->assertArrayHasKey('value', $current);
                $this->assertSame(current($values), $current['value']);
            }
        }
    }
}
