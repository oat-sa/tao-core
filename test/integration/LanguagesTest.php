...
class LanguagesTestCase extends TaoPhpUnitTestRunner
{
     ...
     * @author Jerome Bogaerts, <taosupport@tudor.lu>
     */
    public function testLanguagesExistence(): void
    {
        ...
        if (($locales = scandir($taoLocalesDir)) !== false) {
            foreach ($locales as $l) {
                ...
                if ($l[0] !== '.' && is_dir($localePath) && is_readable($localePath)) {
                    ...
                    if (file_exists($langPath)) {
                        ...
                        if (is_array($kbLangs)) {
                            $this->assertSame(count($kbLangs), 1, "Number of languages retrieved for language '${l}' is '" . count($kbLangs) . "'.");
     ...
     * @author Jerome Bogaerts, <taosupport@tudor.lu>
     */
    public function testLanguageUsages(): void
    {
        ...
        $this->assertSame($lgUsageProperty->getUri(), tao_models_classes_LanguageService::PROPERTY_LANGUAGE_USAGES);
        $usagePropertyRange = $lgUsageProperty->getRange();
        $this->assertIsA($usagePropertyRange, 'core_kernel_classes_Class');
        $this->assertSame($usagePropertyRange->getUri(), tao_models_classes_LanguageService::CLASS_URI_LANGUAGES_USAGES);
        ...
        $this->assertTrue(in_array(DEFAULT_LANG, $defaultLanguage->getPropertyValues($valueProperty), true));

        $usages = $defaultLanguage->getPropertyValues($lgUsageProperty);
        $this->assertTrue(count($usages) >= 2);
        $this->assertTrue(in_array(tao_models_classes_LanguageService::INSTANCE_LANGUAGE_USAGE_GUI, $usages, true));
        $this->assertTrue(in_array(tao_models_classes_LanguageService::INSTANCE_LANGUAGE_USAGE_DATA, $usages, true));
    }
}
