...
class ServiceTest extends TaoPhpUnitTestRunner
{
     ...
     */
    public function testTaoServiceExtention(): void
    {
    ...
    public function testAbstractService(): void
    {
        ...
        $this->assertSame($testProperty->getUri(), $diffProperty->getUri());
        ...
        $testInstance = $this->taoService->bindProperties(
            $testInstance,
            [
                $testProperty->getUri() => ['value' => 'aValue'],
            ]
        );
        $this->assertIsA($testInstance, 'core_kernel_classes_Resource');
        ...
        $this->assertNotSame($clonedInstance->getUri(), $testInstance->getUri());
        $this->assertSame($testInstance->getUniquePropertyValue($testProperty), $clonedInstance->getUniquePropertyValue($testProperty));
        ...
        if (count($itemSubClasses) > 0) {
            foreach ($itemSubClasses as $testClass) {
                break;
            }
        } else {
            $testClass = $itemClass;
     ...
     */
    public function testSetUploadFileSource(): void
    {
     ...
     * @author Lionel Lecaque, lionel@taotesting.com
     */
    public function testFileCacheService(): void
    {
        ...
        $this->assertSame($fromCache, 'string1');
        ...
        $this->assertSame($fromCache, $testarr);
        $fc->remove('testcase3');
