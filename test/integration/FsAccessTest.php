 ...
 * @package tao
 */
class FsAccessTest extends TaoPhpUnitTestRunner
{
    ...
    {
        $this->disableCache();
        $pass = md5(random_int(0, getrandmax()));
     ...
     */
    public function testAddWebSourceException(): void
    {
     ...
     * @author Lionel Lecaque, lionel@taotesting.com
     */
    public function testAddWebSource(): void
    {
     ...
     * @author Lionel Lecaque, lionel@taotesting.com
     */
    public function testRemoveWebSource(): void
    {
        $websourceMock = $this->getWebsourceMock();
        WebsourceManager::singleton()->addWebsource($websourceMock);
     ...
     * @author Lionel Lecaque, lionel@taotesting.com
     */
    public function testRemoveWebSourceException(): void
    {
        $this->expectException(common_Exception::class);
    ...
    public function testDirectWebsourceProvider(): void
    {
        $ext = common_ext_ExtensionsManager::singleton()->getExtensionById('tao');
    ...
    public function testTokenWebsourceProvider(): void
    {
        $ext = common_ext_ExtensionsManager::singleton()->getExtensionById('tao');
        $this->registerFileSystem($ext);
    ...
    public function testActionWebsourceProvider(): void
    {
        $ext = common_ext_ExtensionsManager::singleton()->getExtensionById('tao');
        $this->registerFileSystem($ext);
    ...
    private function registerFileSystem(\common_ext_Extension $ext): void
    {
        $serviceManager = ServiceManager::getServiceManager();
        $fsm = $serviceManager->get(FileSystemService::SERVICE_ID);
    ...
    private function assertUrlHttpCode($url, $expectedCode = 200): void
    {
        $ch = curl_init($url);
        ...
        $this->assertSame($expectedCode, $httpCode, 'Incorrect response for ' . $url);
    }
