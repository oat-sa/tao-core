...
class ConnectorUserServiceTestCase extends TaoPhpUnitTestRunner {
    ...
    public function testLogin() {
    	...
    	$this->setExpectedException('taoSessionRequiredException');
    ...
    public function testGetInfo() {
    	...
    	$data = $this->userService->startSession($this->UserData[PROPERTY_USER_LOGIN], $this->UserData[PROPERTY_USER_PASSWORD]);
    	$this->assertNotEquals($data, false);
    	$this->assertTrue($this->userService->hasSession());
    	...
    	$this->assertTrue(isset($data['id']));
    	$this->assertEquals($this->userUri, $data['id']);

    	$this->assertTrue(isset($data['roles']));
    	$this->assertEquals($roles, $data['roles']);
    	
    	$this->assertTrue(isset($data['lang']));
    	$code = $this->getLangValue($this->UserData[PROPERTY_USER_UILG]);
    	$this->assertEquals($data['lang'], $code);
    ...
    public function testChangePass() {
    	...
    	$data = $this->userService->startSession($this->UserData[PROPERTY_USER_LOGIN], $this->UserData[PROPERTY_USER_PASSWORD]);
    	$this->assertNotEquals($data, false, 'Login failed during '.__FUNCTION__);
    	$this->assertTrue($this->userService->hasSession());
    ...
    public function testChangeLang() {
    	...
    	$this->assertNotEquals($newLang, $oldLang);
    	...
    	$this->assertEquals($newLang, $currentLang);
