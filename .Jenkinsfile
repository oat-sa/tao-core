...
registry.service.consul:4444/tao/dependency-resolver oat:dependencies:resolve --main-branch ${TEST_BRANCH} --repository-name ${REPO_NAME} > build/composer.json
        ...
        stage('Install') {
            ...
            steps {
                dir('build') {
                    sh(
                        label: 'Disable packagist',
                        script: 'composer config repositories.packagist false'
                    )
                    sh(
                        label: 'Install/Update sources from Composer',
                        script: 'COMPOSER_DISCARD_CHANGES=true composer update --no-interaction --no-ansi --no-progress --no-scripts --prefer-source'
                    )
                    sh(
                        label: 'Add phpunit',
