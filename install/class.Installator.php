...
class tao_install_Installator
{
     ...
     * @param array $installData data coming from the install form
     */
    public function install(array $installData): void
    {
        try {
            /**
            ...
            foreach ($configChecker->getComponents() as $c) {
                if (method_exists($c, 'getName') && in_array($c->getName(), $this->getEscapedChecks(), true)) {
     ...
     */
    public static function checkInstallData(array $installData): void
    {
        // instance name
        if (empty($installData['instance_name'])) {
            ...
            throw new tao_install_utils_MalformedParameterException($msg);
        } elseif (preg_match('/\s/u', $installData['instance_name']) === 1) {
     ...
     */
    public function escapeCheck($id): void
    {
     ...
     */
    public function setEscapedChecks(array $escapedChecks): void
    {
    ...
    {
        return in_array($id, $this->getEscapedChecks(), true);
    }
     ...
     * @param array $tags
     */
    public function log($logLevel, $message, $tags = []): void
    {
        if (! is_array($tags)) {
            $tags = [$tags];
        }
