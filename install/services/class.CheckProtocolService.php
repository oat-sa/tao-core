...
/**
 * A Service implementation aiming at checking if the server side can talk 'JSON'...
 * 
 * Please refer to tao/install/api.php for more information about how to call this service.
 */
class tao_install_services_CheckProtocolService extends tao_install_services_Service{
    ...
    public function __construct(tao_install_services_Data $data){
        ...
        if (!isset($content['type']) || empty($content['type']) || $content['type'] !== 'CheckProtocol'){
            throw new InvalidArgumentException("Unexpected type: 'type' must be equal to 'CheckProtocol'.");
        }
    }
    ...
    public function execute(){
        ...
        $data = '{"type": "ProtocolReport", "value": '. (($json) ? 'true' : 'false') . '}';
