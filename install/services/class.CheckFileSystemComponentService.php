...
class tao_install_services_CheckFileSystemComponentService extends tao_install_services_Service implements tao_install_services_CheckService
{
    ...
    protected function checkData()
    {
        ...
        } elseif ($content['type'] !== 'CheckFileSystemComponent') {
            throw new InvalidArgumentException("Unexpected type: 'type' must be equal to 'CheckFileSystemComponent'.");
        } elseif (!isset($content['value']) || empty($content['value']) || count($content['value']) == 0) {
        ...
        } elseif (!isset($content['value']['rights']) || empty($content['value']['rights'])) {
            throw new InvalidArgumentException("Missing data: 'rights' must be provided.");
        } elseif (!isset($content['value']['location']) || empty($content['value']['location'])) {
