...
require(['config'], function(){
    ...
    function initHelp(){
        ...
        install.addHelp('hlp_tao_extension_opcache', 'OPcache improves PHP performance by storing precompiled script bytecode in shared memory, thereby removing the need for PHP to load and parse scripts on each request <br/> http://php.net/manual/en/book.opcache.php');
        install.addHelp('hlp_tao_ini_opcache_load_comments', 'If disabled, documentation comments won\'t be loaded from the opcode cache even if they exist.');
        install.addHelp('hlp_tao_ini_opcache_save_comments', 'If disabled, all documentation comments will be discarded from the opcode cache to reduce the size of the optimised code. Disabling this configuration directive will break application');
