...
class tao_scripts_TaoTranslate extends tao_scripts_Runner
{
    ...
    public function preRun(): void
    {
        ...
        if ($this->options['action'] === null) {
            ...
            if (! in_array($this->options['action'], array_keys($this->getAllowedActions()), true)) {
    ...
    public function actionChangeCode(): void
    {
     ...
     */
    public function preparePOFile(tao_helpers_translation_POFile $poFile, $poEditorReady = false): void
    {
     ...
     * @throws Exception
     */
    public function addManifestsTranslations(tao_helpers_translation_POFile $poFile): void
    {
        ...
        if ($directories === false) {
            ...
            foreach ($directories as $dir) {
                if (is_dir($rootDir . $dir) && ! in_array($dir, $exceptions, true)) {
                    // Maybe it should be read.
                    if (in_array('.*', $exceptions, true) && $dir[0] === '.') {
                        continue;
                    }
        ...
        foreach ($extensionsToCreate as $extension) {
            ...
            if (($extDirectories = scandir(ROOT_PATH)) !== false) {
                ...
                foreach ($extDirectories as $extDir) {
                    ...
                    if (is_dir($extPath) && is_readable($extPath) && $extDir[0] !== '.' && ! in_array($extDir, $dependencies, true) && $extDir !== $extension && $extDir !== 'generis') {
     ...
     * @author Joel Bout, <joel.bout@tudor.lu>
     */
    public function actionCompileAll(): void
    {
     ...
     * @author Joel Bout, <joel.bout@tudor.lu>
     */
    protected function addLanguageToOntology(): void
    {
        ...
        if (file_exists($expectedDescriptionPath)) {
            if (is_readable($expectedDescriptionPath)) {
                ...
                if ($generisAdapterRdf->import($expectedDescriptionPath, null, LOCAL_NAMESPACE) === true) {
     ...
     * @author Joel Bout, <joel.bout@tudor.lu>
     */
    protected function removeLanguageFromOntology(): void
    {
     ...
     * @author Joel Bout, <joel.bout@tudor.lu>
     */
    protected function checkAuthInput(): void
    {
        $defaults = ['user' => null,
            'password' => null, ];
    ...
    protected function checkInputOption(): void
    {
    ...
    protected function checkOutputOption(): void
    {
     ...
     * @author Joel Bout, <joel.bout@tudor.lu>
     */
    private function checkCreateInput(): void
    {
     ...
     * @author Joel Bout, <joel.bout@tudor.lu>
     */
    private function checkUpdateInput(): void
    {
        $defaults = ['language' => null,
            'extension' => null,
            'input' => dirname(__FILE__) . '/../../' . $this->options['extension'] . '/' . self::DEF_INPUT_DIR,
     ...
     * @author Joel Bout, <joel.bout@tudor.lu>
     */
    private function checkUpdateAllInput(): void
    {
     ...
     * @author Joel Bout, <joel.bout@tudor.lu>
     */
    private function checkDeleteAllInput(): void
    {
        $defaults = ['input' => dirname(__FILE__) . '/../../' . $this->options['extension'] . '/' . self::DEF_INPUT_DIR,
            'output' => dirname(__FILE__) . '/../../' . $this->options['extension'] . '/' . self::DEF_OUTPUT_DIR,
            'extension' => null, ];
    ...
    private function checkChangeCodeInput(): void
    {
        $defaults = ['input' => dirname(__FILE__) . '/../../' . $this->options['extension'] . '/' . self::DEF_INPUT_DIR,
            'output' => dirname(__FILE__) . '/../../' . $this->options['extension'] . '/' . self::DEF_OUTPUT_DIR,
            'extension' => null,
     ...
     * @author Joel Bout, <joel.bout@tudor.lu>
     */
    private function checkEnableInput(): void
    {
        $this->checkAuthInput();
        $defaults = ['language' => null,
            'input' => dirname(__FILE__) . '/../../' . $this->options['extension'] . '/' . self::DEF_INPUT_DIR,
     ...
     * @author Joel Bout, <joel.bout@tudor.lu>
     */
    private function checkDisableInput(): void
    {
        $this->checkAuthInput();
        $defaults = ['language' => null];
        $this->options = array_merge($defaults, $this->options);
     ...
     * @author Joel Bout, <joel.bout@tudor.lu>
     */
    private function checkCompileInput(): void
    {
     ...
     * @author Joel Bout, <joel.bout@tudor.lu>
     */
    private function checkCompileAllInput(): void
    {
        $defaults = ['extension' => null,
            'input' => dirname(__FILE__) . '/../../' . $this->options['extension'] . '/' . self::DEF_INPUT_DIR,
            'output' => dirname(__FILE__) . '/../../' . $this->options['extension'] . '/' . self::DEF_OUTPUT_DIR, ];
    ...
    private function actionGetExt(): void
    {
        $rootDir = ROOT_PATH;
        $extensions = [];
        $extensionsList = '';
