...
class com.eXULiS.lib.ListBoxDatasModel {	
	...
	// @soName_str : 
	public function ListBoxDatasModel()
	{		
		trace("[ListBoxDatasModel]");		
		init();		
	}
	...
	{
		trace("[ListBoxDatasModel] addController: controllers_ar.length = " + controllers_ar.length);
		// test if controller is not already in
		for (var i in controllers_ar)
		{
			if (controllers_ar[i]==controller)
			{
				trace("[ListBoxDatasModel] > [addController] this controller already in");
				return false;
			}
		}
	...
	private function loadXMLstring(content_str:String):Void
	{
		...
		trace("[BLACKListBoxDatasModel ---------: " + currentXml_xml);
