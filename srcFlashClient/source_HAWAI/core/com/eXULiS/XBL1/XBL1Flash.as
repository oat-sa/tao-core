...
* @usage data = new XBL1Flash().parseXML(anXML);

*/
class com.eXULiS.XBL1.XBL1Flash extends XML {
...
*/
	function XBL1Flash(target_mc:MovieClip) {
		canvas_mc = target_mc;
//		trace("XBL1Flash: canvas initialized to " + canvas_mc + " (base was " + target_mc + "[" + target_mc._name + "])");
		xblDefsRepository_obj = new defsRepository();
		canvas_mc._exulis._xbl1DefsRepository = xblDefsRepository_obj;
	}
...
//		trace("XBL1Flash: XBL something (" + node.attributes["id"] + ") triggered on " + current_mc._xbl.id + " on depth: " + current_mc._childNextDepth);
//		var object_obj = new XBL1something(current_mc,node);
		var local_mc;
...
/**
* @method translateXML
* @description core of the XBL1Flash class
*/
...
// here unhandled tags
			default:
				trace("XBL1Flash: XBL tag undefined: " + node.nodeName);
				local_mc = current_mc;
		}
		return local_mc;
	}
