...
class TaskLog extends ConfigurableService implements TaskLogInterface
{
    ...
    public function findAvailableByUser($userId, $limit = null, $offset = null)
    {
        $filter = (new TaskLogFilter())
            ...
            ->setLimit($limit === null ? self::DEFAULT_LIMIT : $limit)
     ...
     * @inheritdoc
     */
    public function linkTaskToCategory($taskName, $category): void
    {
        if (is_object($taskName)) {
            $taskName = get_class($taskName);
        }

        if (! in_array($category, $this->getTaskCategories(), true)) {
     ...
     * @param string $status
     */
    protected function validateStatus($status): void
    {
        $statuses = [
     ...
     */
    protected function checkIfCanArchive(EntityInterface $entity, $forceArchive): void
    {
     ...
     * @throws \common_Exception
     */
    protected function checkIfCanCancel(EntityInterface $entity, $forceCancel): void
    {
