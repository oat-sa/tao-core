...
class ThemeRegistry extends AbstractRegistry
{
    ...
    public function setWebSource($websource): void
    {
        $this->set(self::WEBSOURCE, $websource);
    }
     ...
     */
    public function setDefaultTheme($target, $themeId): void
    {
     ...
     * @throws \common_Exception
     */
    public function createTarget($targetId, $base): void
    {
     ...
     * @throws \common_Exception
     */
    public function registerTheme($id, $name, $path = '', $targets = [], $templates = []): void
    {
     ...
     * @throws \common_Exception
     */
    public function unregisterTheme($id): void
    {
        if (preg_match('/^[a-zA-Z0-9]*$/', $id) === 0) {
            throw new \common_Exception('Invalid id "' . $id . '"');
        }
        ...
        $map = $this->getMap();
        unset($map[self::WEBSOURCE]); //still ugly but looks better than 'continue'
    ...
    {
        if (isset($theme['path'])) {
            if (strpos($theme['path'], self::WEBSOURCE) === 0) {
                $websource = WebsourceManager::singleton()->getWebsource($this->get(self::WEBSOURCE));
                $webUrl = $websource->getAccessUrl(substr($theme['path'], strlen(self::WEBSOURCE)));
                $theme['path'] = $webUrl;
            } else {
    ...
    {
        $websource = WebsourceManager::singleton()->getWebsource($this->get(self::WEBSOURCE));
        if (strpos($path, self::WEBSOURCE) === 0) {
            return $websource->getAccessUrl(substr($path, strlen(self::WEBSOURCE)));
        }
